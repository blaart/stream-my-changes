...
echo "Checking health of DynamoDB"
until aws --endpoint-url=http://awssvc:4566 dynamodb list-tables; do
  >&2 echo "DynamoDB is unavailable - sleeping"
  sleep 1
done
echo "DynamoDB OK"
#create the dynamodb's
aws --endpoint-url=http://awssvc:4566 dynamodb create-table \
    --region eu-central-1 \
...
aws --endpoint-url=http://awssvc:4566 dynamodb create-table \
    ...
    --key-schema AttributeName=contractnumber,KeyType=HASH \
    --provisioned-throughput ReadCapacityUnits=5,WriteCapacityUnits=5
echo "Contract2 table created"

until aws --endpoint-url=http://awssvc:4566 lambda list-functions; do
  >&2 echo "Lambda is unavailable - sleeping"
  sleep 1
done

#create the lambda's
aws --endpoint-url http://awssvc:4566 lambda create-function \
...
until aws --endpoint-url=http://awssvc:4566 kinesis list-streams; do
  >&2 echo "Kinesis is unavailable - sleeping"
  sleep 1
done

#create the kinesis streams
aws --endpoint-url=http://awssvc:4566 kinesis create-stream \
    --stream-name contractChanges \
    --shard-count 1

#link the lambda's as consumer of the streams
aws --endpoint-url=http://awssvc:4566 lambda create-event-source-mapping \
    --function-name processKinesisStreamContractChange1 \
    --enabled \
    --batch-size 500 \
